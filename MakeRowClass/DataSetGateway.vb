...
#Region "メソッド"
        ...
        Public Overloads Function FillData(ByVal tableName As String, ByVal selectCommand As DbCommand) As Boolean
            ...
            Catch ex As Exception
                MessageBox.Show(ex.Message, "DataSetGateway.FillData")
            Finally
        ...
        Public Sub CreatePrimaryKey(ByVal strTblName As String, ByVal aclmName() As String)
            ...
            If _Data.Tables.Contains(strTblName) Then
                ...
                If Not IsNothing(tbl) Then
                    ...
                    tbl.PrimaryKey = aClm
                Else
                    MessageBox.Show(strTblName & " Error", "DataSetGateway.CreatePrimaryKey")
                End If
        ...
        Public Sub FillViewData(ByVal tableName As String, ByVal commandText As String)
            ...
            Catch ex As Exception
                MessageBox.Show(ex.Message, "DataSetGateway.FillViewData")
            Finally
                _Cnn.Connection.Close()
            End Try
...
#Region "プロテクト　メソッド"
        ...
        ''' <remarks></remarks>
        Protected Function GetAdapter(ByVal strTblName As String, ByVal transaction As DbTransaction) As DbDataAdapter

            'トランザクション処理をしているのでコマンドとコマンドビルダーを再作成する
            Dim sda As DbDataAdapter = DataAdapter(strTblName)
            mprcReCommandBuilder(sda, transaction)
            Return sda
...
#Region "プライベート　メソッド"
        ...
        ''' <param name="transaction">トランザクション</param>
        ''' <remarks></remarks>
        Private Sub mprcReCommandBuilder(ByVal adapter As DbDataAdapter, ByVal transaction As DbTransaction)
...
#Region "オーバーライド可能　メソッド"
        ...
        Protected Overridable Sub UpdateDataSet(ByVal transaction As DbTransaction)
            ...
            For Each tableName As String In _dataAdapters.Keys
                ...
                'トランザクション処理をしているのでコマンドとコマンドビルダーを再作成する
                mprcReCommandBuilder(adapter, transaction)
